...
namespace {
    ...
    ChunkAnalysis analyze(const std::vector<int16_t>& v, std::size_t frames, float volume) {
        ChunkAnalysis a;
        analyze_chunk(v, frames, volume, a);
        return a;
    }
...
TEST_CASE("無音はすべて 0", "[meter]") {
    auto a = analyze(stereo(64, 0, 0), 64, 1.0f);
    CHECK(a.rms_l == 0.0f);
    CHECK(a.rms_r == 0.0f);
    CHECK(a.peak_l == 0.0f);
    CHECK(a.peak_r == 0.0f);
...
TEST_CASE("フルスケールはピーク 1.0 でクリップ扱い", "[meter]") {
    auto a = analyze(stereo(32, 32767, -32768), 32, 1.0f);
