...
namespace acr {
    ...
    void sort_devices(std::vector<DeviceInfo>& sources) {
        std::sort(sources.begin(), sources.end(), [](const DeviceInfo& a, const DeviceInfo& b) {
    ...
    DeviceMatch match_device(const std::vector<DeviceInfo>& sources, const std::string& source_arg) {
        DeviceMatch result;
        if (sources.empty()) {
            result.status = DeviceMatch::Status::Empty;
            return result;
        }

        if (source_arg.empty()) {
            auto it = std::find_if(sources.begin(), sources.end(), [](const DeviceInfo& s) {
                return s.is_default;
            });
            result.status = DeviceMatch::Status::Ok;
        ...
        auto exact = std::find_if(sources.begin(), sources.end(), [&](const DeviceInfo& s) {
        ...
        if (matches.empty()) {
            result.status = DeviceMatch::Status::NoMatch;
        } else {
            result.status = DeviceMatch::Status::Ambiguous;
